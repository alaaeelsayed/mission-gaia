...
namespace wolf
{
	...
	//----------------------------------------------------------
	void VertexDeclaration::End(int stride /* override if you don't want it to calculate itself */)
	{
		if (m_pVB)
			m_pVB->Bind();
		if (m_pIB)
		...
		for (unsigned int i = 0; i < m_attrs.size(); i++)
		{
			...
			bool isColor = info.m_attr == AT_Color;
			glVertexAttribPointer(info.m_attr,
								  info.m_type == CT_UByte4 ? 4 : info.m_iNumComponents,
