...
namespace wolf
{
    ...
    Program::Program(const std::string &sFile, GLenum shaderType)
    {
        GLchar *pFileContents = (GLchar *)wolf::LoadWholeFile(sFile);
        ...
        case GL_VERTEX_SHADER:
            printf("Failed to add %s. A vertex shader already exists.", sFile.c_str());
            break;

        case GL_FRAGMENT_SHADER:
            printf("Failed to add %s. A fragment shader already exists.", sFile.c_str());
            break;

        case GL_TESS_CONTROL_SHADER:
            m_tcsSrc = pFileContents;
            break;

        case GL_TESS_EVALUATION_SHADER:
            m_tesSrc = pFileContents;
            break;

        case GL_GEOMETRY_SHADER:
            m_gsSrc = pFileContents;
            break;

        case GL_COMPUTE_SHADER:
            m_csSrc = pFileContents;
            break;
        }
    }
    ...
    //----------------------------------------------------------
    // Sets a texture uniform of the given name
    //----------------------------------------------------------
    void Program::SetTexture(const std::string &name, const wolf::Texture *pTex)
    {
    ...
    //----------------------------------------------------------
    // Dispatches compute shader with group size
    //----------------------------------------------------------
    void Program::DispatchCompute(unsigned int uiNumGroupsX, unsigned int uiNumGroupsY, unsigned int uiNumGroupsZ)
    {
        glDispatchCompute(uiNumGroupsX, uiNumGroupsY, uiNumGroupsZ);
    }

    //----------------------------------------------------------
    // Creates shader of shaderType
    //----------------------------------------------------------
    void Program::addShader(const std::string &sFile, GLenum shaderType)
    {
        GLchar *pFileContents = (GLchar *)wolf::LoadWholeFile(sFile);
    ...
    void Program::_compile(std::size_t variantKey, const std::set<std::string> &keywords)
    {
        ...
        if (!m_tcsSrc.empty())
        {
            if (!_compileShader(&uiTCS, GL_TESS_CONTROL_SHADER, preamble, m_tcsSrc))
            {
        ...
        if (!m_tesSrc.empty())
        {
            if (!_compileShader(&uiTES, GL_TESS_EVALUATION_SHADER, preamble, m_tesSrc))
            {
        ...
        if (!m_gsSrc.empty())
        {
            if (!_compileShader(&uiGS, GL_GEOMETRY_SHADER, preamble, m_gsSrc))
            {
        ...
        if (!m_csSrc.empty())
        {
            if (!_compileShader(&uiCS, GL_COMPUTE_SHADER, preamble, m_csSrc))
            {
        ...
        if (!m_csSrc.empty())
            glAttachShader(glProgram, uiCS);
        if (!m_tcsSrc.empty())
            glAttachShader(glProgram, uiTCS);
        if (!m_tesSrc.empty())
            glAttachShader(glProgram, uiTES);
        if (!m_gsSrc.empty())
            glAttachShader(glProgram, uiGS);
        ...
        if (!_linkProgram(glProgram))
        {
            ...
            if (!m_vsSrc.empty() && uiVS)
            {
                glDeleteShader(uiVS);
            }
            if (!m_fsSrc.empty() && uiPS)
            {
                glDeleteShader(uiPS);
            }

            if (!m_csSrc.empty() && uiCS)
            {
                glDeleteShader(uiCS);
            }

            if (!m_tcsSrc.empty() && uiTCS)
            {
                glDeleteShader(uiTCS);
            }

            if (!m_tesSrc.empty() && uiTES)
            {
                glDeleteShader(uiTES);
            }

            if (!m_gsSrc.empty() && uiGS)
            {
                glDeleteShader(uiGS);
            }
        ...
        // Release vertex, fragment, and compute shaders.
        if (!m_vsSrc.empty() && uiVS)
            glDeleteShader(uiVS);
        if (!m_fsSrc.empty() && uiPS)
            glDeleteShader(uiPS);
