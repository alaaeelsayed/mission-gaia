...
//
// Edited by: Hesham ElHamshary
//
// Added functionality for a compute shader
//-----------------------------------------------------------------------------
#ifndef W_PROGRAM_H
#define W_PROGRAM_H
...
namespace wolf
{
	...
	public:
		...
		void DispatchCompute(unsigned int uiNumGroupsX, unsigned int uiNumGroupsY, unsigned int uiNumGroupsZ);

		// Allow adding optional shaders
		void addShader(const std::string &sFile, GLenum shaderType);

		// Allow adding texture
		void SetTexture(const std::string &name, const wolf::Texture *);
	...
	private:
		...
		Program(const std::string &vsFile, const std::string &fsFile); // For non-compute shaders
		Program(const std::string &sFile, GLenum shaderType);		   // For compute shaders
		virtual ~Program();
		...
		// Shaders
		std::string m_vsSrc;  // Vertex Shader
		std::string m_fsSrc;  // Fragment Shader
		std::string m_tcsSrc; // Tesselation Control Shader
		std::string m_tesSrc; // Tesselation Evaluation Shader
		std::string m_gsSrc;  // Geometry Shader
		std::string m_csSrc;  // Compute Shader
