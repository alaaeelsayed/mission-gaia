...
namespace wolf
{
	...
	// resulting shader object in pShader. Returns true on success,
	// else false
	//----------------------------------------------------------
	bool CompileShader(GLuint *pShader, GLenum shaderType, const std::string &filename)
	{
		GLint iStatus;
		const GLchar *pSource;

		pSource = (GLchar *)LoadWholeFile(filename);
		if (!pSource)
		{
	...
	GLuint LoadShaders(const std::string &vsFile, const std::string &fsFile)
	{
		...
		if (!CompileShader(&uiVS, GL_VERTEX_SHADER, vsFile))
		{
		...
		if (!CompileShader(&uiPS, GL_FRAGMENT_SHADER, fsFile))
		{
...
#if 0
	GLFWimage img;
	if( GL_FALSE == glfwReadImage( filename.c_str(), &img, 0 ) )
		return false;
...
#define GL_COMPRESSED_RGBA_S3TC_DXT5_EXT 0x83F3
	...
	GLuint CreateTextureFromDDS(const std::string &filename, unsigned int *pWidth, unsigned int *pHeight, bool *pHasMips)
	{
		DDS_TEXTURE dds;
		memset(&dds, 0, sizeof(dds));

		unsigned char *pBuff = (unsigned char *)LoadWholeFile(filename);
		if (!pBuff)
			return 0;
		...
		{
			printf("%s is not a dds file", filename.c_str());
			return 0;
		}
		...
		{
			printf("ERROR: %s is a volume texture ", filename.c_str());
			return 0;
		}
