...
namespace wolf
{
	...
	//----------------------------------------------------------
	Texture::Texture(void *pData, unsigned int width, unsigned int height, Format internalFmt, Format fmt)
		: m_minFilter(Texture::FM_Invalid), m_magFilter(Texture::FM_Invalid),
		  m_uWrap(Texture::WM_Invalid), m_vWrap(Texture::WM_Invalid),
		  m_width(0), m_height(0), m_glTex(0)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, gs_aFormatMap[internalFmt], width, height, 0, fmt == Texture::FMT_INVALID ? gs_aFormatMap[internalFmt] : gs_aFormatMap[fmt], gs_aTypeMap[fmt], pData);
	...
	void Texture::BindImageTexture(unsigned int unit, int level, bool layered, int layer, GLenum access, Format fmt) const
	{
		// glActiveTexture(GL_TEXTURE0 + unit);
		glBindImageTexture(unit, m_glTex, level, layered, layer, access, gs_aFormatMap[fmt]);
	}
