...
const Vertex Water::gs_surfaceVertices[6] = {
...
Water::Water()
{
    if (!m_bInitialized)
    {
        ...
        // Instantiate Buffers
        m_pVB = wolf::BufferManager::CreateVertexBuffer(gs_surfaceVertices, sizeof(Vertex) * 6);
        _generateIndices();
...
void Water::render(const glm::mat4 &mProj, const glm::mat4 &mView, int width, int height)
{
    ...
    // Bind Uniforms
    m_pRenderProgram->SetUniform("projection", mProj);
    m_pRenderProgram->SetUniform("view", mView);
    m_pRenderProgram->SetUniform("world", mWorld);
    m_pRenderProgram->SetUniform("worldIT", glm::transpose(glm::inverse(mWorld)));
    m_pRenderProgram->SetTexture("u_diffuseTex", m_pDy);
...
void Water::_calculateH0T(float dt)
{
    // Compute Spectrum texture over time
    m_pAmplitudeByTimeProgram->SetUniform("L", L);
    m_pAmplitudeByTimeProgram->SetUniform("t", dt);
